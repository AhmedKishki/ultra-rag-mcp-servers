...
| [`memory-ultra-rag-mcp-server`](memory-ultra-rag-mcp-server/) | A project-scoped, strongly typed store of objects and relations for durable operational memory between agent sessions: the MCP reference memory server's entities, relations, and observations reimplemented in Python, extended with a project-declared type schema in `project-types.json`, a bounded read surface with no whole-store read, one retrieval boundary with declared-type filters and verdicts that disclose what they searched, admission control that refuses an exact duplicate with the statements it conflicts with and records the reason the user gave when it is overridden, reasoned reversible withdrawal, supersession, confirmed deletion, and documented conventions for recording a project's own files as typed reference objects rather than reading them, stored per project in SQLite with a trigger-maintained FTS5 index. | You want an agent to carry rules, decisions, preferences, and open work across sessions instead of retrieving documents. The global store, session tiers, operative rules, relations as addressable atoms, and export/import are still planned. |
