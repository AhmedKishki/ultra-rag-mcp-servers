...
| [`memory-ultra-rag-mcp-server`](memory-ultra-rag-mcp-server/) | UltraRAG's own memory served unmodified over stdio MCP: one global `MEMORY.md` per user and one dated dialogue file per day, with upstream's two tools (`get_global_memory`, `save_memory`), file formats, and storage layout. The pinned UltraRAG server runs as a child process and is proxied, so no upstream source is copied; its storage root is passed in, which is what makes the memory land where a UltraRAG UI reads it. | You want the memory UltraRAG itself keeps — a small whole-document profile plus an append-only dialogue log — exposed to an agent, or you want that memory visible in the UltraRAG UI. It has no search, no types, no deduplication, and no correction. |
