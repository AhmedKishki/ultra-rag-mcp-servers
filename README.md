...
| [`memory-ultra-rag-mcp-server`](memory-ultra-rag-mcp-server/) | UltraRAG's memory over stdio MCP, in two kinds: **local memory**, kept inside the project repository under `.memory-rag` so it travels with the project, and **global memory**, kept in UltraRAG's shared storage tree so every instance and the UltraRAG UI read the same one. Each kind is a standing `MEMORY.md` plus one dated dialogue file per day, in UltraRAG's own format; four tools: `get_local_memory`, `save_local_memory`, `get_global_memory`, `save_memory`. An optional browser view of both kinds, built on `ui-ultra-rag-mcp`, is served by `memory-ultra-rag-ui` or by the server itself on `--ui-port`. | You want an agent to carry memory across sessions, with each project's memory isolated in its own repository and a shared memory for what applies everywhere. |
