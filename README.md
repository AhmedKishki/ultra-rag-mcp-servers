...
| [`memory-ultra-rag-mcp-server`](memory-ultra-rag-mcp-server/) | A project-scoped, strongly typed store of objects, relations as addressable atoms, and atomic statements for durable operational memory between agent sessions: the MCP reference memory server's entities, relations, and observations reimplemented in Python, extended with a project-declared type schema in `project-types.json`, a canonical vocabulary of research and writing types that a project declares in one call and edits by hand, a bounded read surface with no whole-store read, one retrieval boundary with declared-type filters and verdicts that disclose what they searched, admission control that refuses an exact duplicate with the statements it conflicts with and records the reason the user gave when it is overridden, reasoned reversible withdrawal, supersession, confirmed deletion, bulk relation by declared type behind a preview and a cap, one separate global store per user for cross-project rules, preferences, and facts behind an explicit `scope` on every write and removal and a labelled `include_global` group on every read, store-kind verification that refuses a ledger copied into the other store's place, a reviewed `promote_to_global` that copies named objects across that line while refusing a project-relative reference and reporting every edge it skipped, identifiers that are never reused so a cited identifier keeps its meaning, and documented conventions for recording a project's own files as typed reference objects rather than reading them — so a relation can carry the statements that document it, and stored objects can carry what a project formerly kept in memory Markdown, stored per project in SQLite with a trigger-maintained FTS5 index. | You want an agent to carry rules, decisions, preferences, dependencies, and open work across sessions instead of retrieving documents. Session tiers, operative rules, approval states, multi-hop traversal, and export/import are still planned. |
